...
/*
 * Area chart report card (reaviz + framer-motion).
 * Original "Incident Report" demo is kept as the default props, so
 * <IncidentReportCard /> with no props renders exactly like the source demo.
 * Dan Gate change: everything (title, legend, series, colours, metrics) can be
 * passed in as props, and `style jsx` was replaced with a plain <style> tag so it
 * does not depend on styled-jsx.
 */
...
type TrendComponent = React.FC<{ baseColor: string; strokeColor: string; className?: string }>;

const DiamondAlertIcon: IconComponent = ({ className }) => (
...
const CircleAlertIcon: IconComponent = ({ className }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
const TriangleAlertIcon: IconComponent = ({ className }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
export const UpTrendIcon: TrendComponent = ({ baseColor, strokeColor, className }) => (
...
export const DownTrendIcon: TrendComponent = ({ baseColor, strokeColor, className }) => (
  <svg className={className} width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect width="28" height="28" rx="14" fill={baseColor} fillOpacity="0.4" />
...
export interface IncidentReportCardProps {
  title?: string;
  /** Small line under the title (e.g. a data disclaimer). */
  subtitle?: string;
  legend?: LegendItem[];
  series?: ChartSeries[];
  /** Colours for each series, in the same order as `series`. */
  colorScheme?: string[];
  metrics?: MetricInfo[];
  /** Tick label formatter for the x axis. */
  formatTick?: (v: Date) => string;
  /** Animate the metric rows only when the card scrolls into view. */
...
const IncidentReportCard: React.FC<IncidentReportCardProps> = ({
  ...
  formatTick = (v) => new Date(v).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' }),
  animateOnView = false,
  className,
}) => {
  const chartData = React.useMemo(() => validateChartData(series), [series]);
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'flex flex-col pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-md min-h-[580px] overflow-hidden transition-colors duration-300',
          className,
        )}
      >
        <h3 className={cn('text-3xl text-left p-7 pt-6 font-bold text-black dark:text-white transition-colors duration-300', subtitle ? 'pb-1' : 'pb-8')}>
          {title}
        </h3>
        {subtitle && <p className="px-7 pb-6 text-xs text-gray-500 dark:text-gray-400">{subtitle}</p>}
          ...
          <AreaChart
            ...
            xAxis={
              <LinearXAxis
                ...
                tickSeries={
                  <LinearXAxisTickSeries
                    label={<LinearXAxisTickLabel format={(v) => formatTick(new Date(v))} fill="var(--reaviz-tick-fill)" />}
                    tickSize={10}
                  />
                }
              />
