 ...
 *
 * Dan Gate additions (shaders untouched):
 * - `transparent` — outputs alpha from brightness so the effect sits on any
 *   background with no black box.
 * - Pauses rendering while off-screen, and draws a single still frame when the
...
/**
 * Wraps a shader so its output alpha follows brightness (black -> fully
...
export function SiriWave({
  ...
  return (
    <canvas
      ...
      className={cn(
        "block",
        transparent ? "bg-transparent" : "rounded-[20px] bg-black",
        className,
      )}
